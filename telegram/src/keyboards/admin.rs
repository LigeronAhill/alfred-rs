...
use crate::callbacks::{
    ADMIN, ADMIN_CALLBACK, CALCULATE, CALCULATE_CALLBACK, LIST_POSTS, LIST_POSTS_CALLBACK,
    NEW_POST, NEW_POST_CALLBACK, NEWS, NEWS_CALLBACK, PRODUCTS, PRODUCTS_CALLBACK,
};

pub(super) fn main_menu_keyboard() -> KeyboardMarkup {
    let news_button = KeyboardButton::new(NEWS);
    let admin_button = KeyboardButton::new(ADMIN);
    ...
    let result = KeyboardMarkup::default()
        .append_row(vec![news_button, admin_button])
...
pub(super) fn main_menu_inline_keyboard() -> InlineKeyboardMarkup {
    ...
    let admin_button = InlineKeyboardButton::callback(ADMIN, ADMIN_CALLBACK);
